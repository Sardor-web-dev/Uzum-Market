<template>
  <div id="add-favorite" class="container">
    <div class="content">
      <h1 v-if="favorites.length > 0" class="favorite-text">Избранное</h1>
      <hr v-if="favorites.length > 0" class="favorite-line">
      <div v-else class="else-content">
        <img class="icon-favorite" src="/src/assets/imgs/hearts 1.png" alt="Избранное пусто" />
        <p class="main-text">Добавьте то, что понравилось</p>
        <p class="sub-text">Перейдите на главную страницу и нажмите на ♡ в товаре</p>
      ...
      <div v-if="favorites.length > 0 " class="cards">
        ...
        <button  class="like"  @click="favorite(product.id)" :onclick="'document.getElementById(\'product' + product.id +'\').style.display = \'none\';' ">
          
          <svg 
            data-v-04a70945="" width="20" height="20" viewBox="0 0 19 16" fill="none" xmlns="http://www.w3.org/2000/svg" alt="like" class="ui-icon ">
      ...
      <div class="details">
        ...
        <div class="card-footer">
          ...
          <button class="basket" @click="addToCart(product)">
      ...
      fetch('/dbj.json')
        ...
        .then(data => {
          ...
          this.products.forEach(product => {
            if (this.likedProducts.includes(product.id)) {
